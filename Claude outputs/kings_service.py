...
def _format_list_line(row) -> str:
    ...
    reign_part = f"règne {reign_start}–{reign_end}"
    if reason:
        reign_part += f" ({reason})"
...
async def get_king_answer(message: str) -> dict:
    ...
    try:
        if not raw_query:
            ...
            # (demande explicite), pas un simple message d'aide.
            return {"text": get_king_list_text(conn), "king": None}
        ...
        if len(rows) > 1:
            options = ", ".join(
                f"{_display_name(r)} ({r['reign_start']}-{r['reign_end']})" for r in rows
            )
            return {
                "text": (
